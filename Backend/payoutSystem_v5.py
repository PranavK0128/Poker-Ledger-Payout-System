...
# ──────────────────────────
# • Handles duplicate names by appending “@row<i>” internally.
# • Greedy largest-debtor ↔ largest-creditor matching.
# • Hard cap: **3 outgoing transfers** per ledger row.
# • Any overflow is sent to BANK, which then fans out to creditors.
# • One-pass 3-cycle cancellation to trim round-robin edges.
# • No rounding — every cent preserved.
# """
...
# getcontext().prec = 28           # plenty of precision for cents
# CENT = Decimal("0.01")
...
#     """
#     Build  { normalised_name → ["(Venmo: @foo)", "(Cashapp: $bar)", …] }.
#     """
#     if not path.exists():
#         return {}
#     df = pd.read_csv(path)
...
#         key = str(row["Player Name"]).strip().lower()
#         handles = []
...
# def method_string(receiver: str, amt: Decimal, db: dict) -> str:
#     if receiver == "BANK":
#         return f"Internal bank transfer of ${money_str(amt)}"
#     handles = db.get(receiver.lower(), []) or ["Venmo: "]
...
#         node = f"{disp} @row{idx}"          # unique per ledger line
...
#             if ending > 0:                  # player cashed out chips
...
# # ─────────────────────── greedy settlement w/ cap = 3 ───────────────────── #

# Transfer = Tuple[str, str, Decimal]          # (from, to, amount)

# def settle(creditors_in, debtors_in, max_out=3) -> List[Transfer]:
#     creditors = deque(sorted([(n, Decimal(a)) for n, a in creditors_in],
#                              key=lambda x: x[1], reverse=True))
#     debtors   =        sorted([(n, Decimal(a)) for n, a in debtors_in],
#                              key=lambda x: x[1], reverse=True)
...
#             # LAST slot → either pay creditor in full or dump to BANK
#             if slots == 1:
#                 if need >= owe:
#                     pay = owe
...
#     # BANK pays any leftover creditors
...
#         for (a, b) in edges:
#             if a == "BANK" or b == "BANK":      # keep BANK edges
#                 continue
#             for (b2, c) in edges:
#                 if b2 != b or c == "BANK":
#                     continue
...
#     # merge duplicate edges now that cycles are gone
